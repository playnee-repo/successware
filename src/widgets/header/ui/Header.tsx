...
export function Header({ project, iteracoes, activeIteracao, disciplina }: HeaderProps) {
  ...
  return (
    <header className="flex items-center gap-2 px-5 py-0 border-b border-border bg-background/80 backdrop-blur-sm shrink-0 z-10 h-12">
      {/* Breadcrumb left */}
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button
            variant="outline"
            size="sm"
            className="gap-2 text-xs h-7 px-2.5 bg-card border-border hover:bg-accent hover:border-border max-w-[220px]"
          >
            {activeIteracao ? (
              <>
                <span className={cn(
                  ...
                  activeIteracao.status === 'concluida' ? 'bg-blue-500' : 'bg-muted-foreground'
                )} />
                <span className="truncate text-foreground">{activeIteracao.nome}</span>
              </>
            ) : (
              <span className="text-muted-foreground">Selecionar iteração</span>
            )}
        ...
        <DropdownMenuContent align="end" className="w-64 bg-popover border-border">
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <span className={cn(
                  ...
                  iter.status === 'concluida' ? 'bg-blue-500' : 'bg-muted-foreground'
                )} />
                <span className="truncate text-foreground">{iter.nome}</span>
              </div>
              {iter.status === 'ativa' && (
          ...
          <DropdownMenuSeparator className="bg-border" />
          <DropdownMenuItem
            className="gap-2 text-xs cursor-pointer text-primary hover:bg-accent focus:bg-accent focus:text-primary"
      ...
      <div className="flex items-center gap-3 px-2">
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground font-medium">Supabase</span>
        </div>
        <div className="flex items-center gap-1.5">
      ...
      {/* Export PDF */}
      <ExportPDFButton project={project} iteracao={activeIteracao} />
      ...
      {/* Notification bell */}
      <button className="relative flex items-center justify-center w-7 h-7 rounded-md text-muted-foreground hover:text-foreground hover:bg-accent transition-all">
      ...
      <Dialog open={showNewIteracao} onOpenChange={setShowNewIteracao}>
        <DialogContent className="max-w-md bg-card border-border">
          <DialogHeader>
            <DialogTitle className="text-foreground">Nova Iteração</DialogTitle>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                className="mt-1.5 bg-muted border-input text-foreground placeholder:text-muted-foreground focus-visible:ring-ring"
              />
            </div>
            <div>
              <label className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">
